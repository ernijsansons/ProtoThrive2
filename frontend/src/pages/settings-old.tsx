...
const SettingsPage: React.FC = () => {
  ...
  const { theme, setTheme, toggleTheme, isEliteMode, setEliteMode, colors } = useTheme();
    ...
    return (
      ...
      >
        <motion.div
          ...
          style={{
            borderTopColor: colors.neonBluePrimary,
            borderRightColor: colors.neonBluePrimary
          }}
        />
        <span
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <button
                className="bg-transparent border border-white/20 text-white px-4 py-2 rounded-lg hover:bg-white/10 flex items-center gap-2 mr-6"
                onClick={() => router.push('/')}
              >
            ...
            <div className="flex items-center space-x-4">
              {/* Theme Toggle */}
              <button
                className="bg-transparent border border-white/20 text-white px-4 py-2 rounded-lg hover:bg-white/10 flex items-center gap-2"
                onClick={toggleTheme}
              >
                ...
                {tabs.map((tab) => {
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        background: isActive
                          ? `linear-gradient(135deg, ${colors.neonBluePrimary}20, ${colors.neonPurple}10)`
                          : 'transparent',
                        color: isActive ? colors.neonBluePrimary : colors.textSecondary,
                        border: isActive ? `1px solid ${colors.neonBluePrimary}40` : '1px solid transparent'
                      }}
                      whileHover={{
                        scale: 1.02,
                        backgroundColor: isActive ? undefined : colors.bgTertiary
                      }}
                      whileTap={{ scale: 0.98 }}
                    >
                      <Icon className="w-5 h-5 mr-3" />
                      ...
                      {isActive && (
                        <motion.div
                          ...
                          style={{ background: colors.neonBluePrimary }}
                          layoutId="activeTab"
                        />
                      )}
                    </motion.button>
            ...
            >
              {activeTab === 'security' && (
                <div className="bg-gray-800/50 backdrop-blur-sm border border-white/10 rounded-lg p-6">
                  <div className="mb-8">
                    <h2
                      className="text-2xl font-bold mb-2 bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent"
                    >
                      Security Settings
                    </h2>
...
export default SettingsPage;// Add getStaticProps for static export
