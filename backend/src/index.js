...
import { validateRoadmapBody, validateUpdateRoadmapBody, validateSnippetBody, validateQueryParams, formatValidationError, ValidationError, RoadmapQuerySchema, SnippetQuerySchema } from './utils/validation';
import { initializeJWTService, createAuthMiddleware, createRateLimitMiddleware, createSecurityHeadersMiddleware, getJWTService } from './utils/auth';
...
app.use('*', async (c, next) => {
    ...
    if (!container) {
        ...
        container = configureContainer(dbService.database, environment);
...
function getAuthMiddleware(requiredRole) {
    try {
...
app.onError((err, c) => {
    ...
    const errorCode = err.code || 'ERR-500';
