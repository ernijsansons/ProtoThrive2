...
import { RATE_LIMIT_CONFIGS, RateLimitConfig } from '../durable-objects/RateLimiter';
 ...
 */
export function createDurableRateLimitMiddleware(options: RateLimitOptions = {}) {
...
export function createSmartRateLimitMiddleware() {
  return createDurableRateLimitMiddleware({
    ...
    onLimitReached: async (c: Context) => {
...
export function createBurstProtectionMiddleware(options: {
  ...
  return createDurableRateLimitMiddleware({
    ...
    keyGenerator: (c: Context) => `burst:${getClientId(c)}`,
    onLimitReached: async (c: Context) => {
      return c.json({
