...
export const RoadmapQuerySchema = z.object({
    limit: z.string().regex(/^\d+$/).transform(Number).max(100).optional(), // SECURITY: Limit query results
...
export function validateRoadmapBody(data) {
    try {
        ...
        if (error instanceof z.ZodError) {
            ...
            return {
                success: false,
                error: new ValidationError(firstError.path.join('.'), 'INVALID_INPUT', firstError.message)
            };
        }
        return {
            success: false,
...
export function validateUpdateRoadmapBody(data) {
    try {
...
export function validateSnippetBody(data) {
    try {
...
export function validateQueryParams(data, schema) {
    try {
        ...
        if (error instanceof z.ZodError) {
            ...
            return {
                success: false,
                error: new ValidationError(firstError.path.join('.'), 'INVALID_QUERY', firstError.message)
            };
        }
        return {
            success: false,
...
export function validateRegisterBody(data) {
    try {
...
export function createBodySizeLimitMiddleware(maxSizeBytes = 1024 * 1024) {
    return async (c, next) => {
...
export function createSecureErrorResponse(code, statusCode, requestId, details) {
    const response = {
...
export function logSecurityEvent(event, severity, context = {}) {
    const sanitizedContext = Object.fromEntries(Object.entries(context).map(([key, value]) => {
