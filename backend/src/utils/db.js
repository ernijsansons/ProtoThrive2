...
export function validateDatabaseBinding(db) {
    if (!db) {
...
export class DatabaseService {
    ...
    async queryRoadmaps(userId, params = {}) {
        try {
            if (!userId) {
    ...
    async insertRoadmap(userId, data) {
        try {
            if (!userId) {
    ...
    async updateRoadmap(roadmapId, userId, data) {
        if (!userId || !roadmapId) {
            throw new Error('User ID and Roadmap ID are required');
        }
        try {
    ...
    async updateRoadmapData(id, data) {
        try {
          ...
          WHERE id = ?
        `)
                .bind(data.name || data.title, data.description || '', jsonGraph, data.thriveScore || 0, now, id)
                .run();
    ...
    async getRoadmaps(userId, limit = 50, offset = 0) {
        try {
            ...
            const result = await this.db
                ...
                .all();
            if (this.kv) {
                await this.kv.put(`roadmaps:${userId}:${limit}:${offset}`, JSON.stringify(result), { expirationTtl: 300 });
            }
            return result;
        }
        catch (error) {
    ...
    async createRoadmap(data) {
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `)
                .bind(id, data.userId, data.name || data.title, data.description || '', jsonGraph, data.thriveScore || 0, now, now)
                .run();
    ...
    // Snippet operations
    async getSnippets(category, limit = 50, offset = 0) {
        try {
    ...
    async createSnippet(data) {
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `)
                .bind(id, data.title, data.code, data.language, data.category || 'general', JSON.stringify(data.tags || []), now, now)
                .run();
    ...
    async createUser(data) {
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
                .bind(id, data.email.toLowerCase(), data.firstName || data.name || '', data.lastName || '', data.passwordHash, role, now, now, false)
                .run();
            return {
                id,
