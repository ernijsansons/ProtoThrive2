...
import { calculateThriveScore } from '../utils/mocks';
...
export const useStore = create<State>((set) => ({
  ...
  calculateScore: () => {
    // Use unified Thrive Score calculation
    const dummyLogs = [
      { status: 'success', type: 'ui' },
      { status: 'success', type: 'api' },
      { status: 'pending', type: 'deploy' }
    ];
    const result = calculateThriveScore(dummyLogs);
    set({ thriveScore: result.score });
    console.log('Thermonuclear Score Calculated:', result);
    return result;
  }
}));