#!/usr/bin/env python3
"""
ProtoThrive Debug & Ship CrewAI System
...
Usage: python protothrive_debug_ship.py
"""
...
# Global dummy data (Ref: CLAUDE.md Section 1)
DUMMY_DATA = {
...
class DebuggerAgent(Agent):
    ...
    def __init__(self):
        super().__init__(
            role="Debugger Agent",
            goal="Analyze code for bugs, lint errors, test failures, and runtime issues",
    ...
    def analyze_codebase(self, project_path: str = ".") -> List[DebugResult]:
        ...
        print("🔍 Thermonuclear Debug: Starting codebase analysis")
    ...
    def _determine_severity(self, issues: List[str]) -> str:
        ...
        if any("critical" in issue.lower() for issue in issues):
            return "critical"
        elif any("security" in issue.lower() for issue in issues):
            return "high"
        elif len(issues) > 5:
            return "medium"
        else:
...
def main():
    ...
    print("ProtoThrive Debug & Ship CrewAI System")
    print("Ref: CLAUDE.md Sections 1-5")
